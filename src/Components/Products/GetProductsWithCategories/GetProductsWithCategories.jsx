...
const GetProductsWithCategories = ({ data }) => {
  const [products, setProducts] = useState([]);
  const [IsLoading, setLoading] = useState(false);
  const [page, setPage] = useState(data?.IsPage || 1);
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      setLoading(true);
      try {
        const response = await findProductsWithCategories(categories, page);
...
export default GetProductsWithCategories;
